...
class Grafo {
	...
	//checar funcionamiento de la pila 
    ...
    public Grafo(int orden){
		...
		//checar vertice como objeto o como arreglo
	...
	//revisar el siguiento polimorifismo 
	public void agregarVertice(Point2D p){		
		Vertice v = new Vertice(p);
		vertices.add(v);
	    System.out.println(mostrarVertices());
    } 
    ...
    public void CMC() { //Dijkstra
    	...
    	if(inicio[0] != null && fin[0] != null) { //Para que comienze, inicio[0] (vertice inicial) y fin[0] (vertice final) deben ser != null
	    	...
	    	while(vi.isVisitado() == false) {
	    		...
	    		if(flag == false) {
	    			for(i=0; i<vertices.size(); i++)
	   					if(vertices.get(i).isVisitado() == false && dv.get(i) < 1000.0)
	   						aux.add(dv.get(i));
	    			if(aux.size() > 0) {
  ...
  //crea el archivo en disco, recibe como parÃ¡metro la lista de Vertices
  		...
  		public void escribirAristas() {
  			...
  			try {
  				...
  				for(int i=0; i<vertices.size(); i++)
  					for(int j=0; j<vertices.size(); j++)
  						if(M[i][j] != null) {
  							bfwriter.write(M[i][j].getPi().getX() + "," + M[i][j].getPi().getY() + "," + M[i][j].getPf().getX() + "," + M[i][j].getPf().getY() + "," + M[i][j].getPeso() + "," + "\n");
  						}
  				//cierra el buffer intermedio
  				bfwriter.close();
  				System.out.println("Archivo creado satisfactoriamente..");
  	...
  	//crea el archivo en disco, retorna la lista de estudiantes
  		public void leerVeritces() {
  			...
  			InputStream file = getClass().getResourceAsStream("/archivos/Vertices.txt");
  			Scanner scanner;
  	...
  	//crea el archivo en disco, retorna la lista de estudiantes
  		public void leerAristas() {
  			// crea el flujo para leer desde el archivo
  			InputStream file = getClass().getResourceAsStream("/archivos/Aristas.txt");
  			Scanner scanner;
  			//se pasa el flujo al objeto scanner
			scanner = new Scanner(file);
			while (scanner.hasNextLine()) {
				...
				//parte la cadena recibida cada vez que encuentre una coma				
				delimitar.useDelimiter("\\s*,\\s*");
				agregarArista(new Point2D.Double(Double.parseDouble(delimitar.next()),Double.parseDouble(delimitar.next())), new Point2D.Double(Double.parseDouble(delimitar.next()),Double.parseDouble(delimitar.next())), Double.parseDouble(delimitar.next()));
			}
			//se cierra el ojeto scanner
			scanner.close();
  		}
